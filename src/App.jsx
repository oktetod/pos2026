...
// === IMPORT KOMPONEN PECAHAN (Akan kita buat setelah ini) ===
...
export default function App() {
  ...
  if (!currentUser) {
    // UI LOGIN (Tetap menggunakan style original agar tidak merusak tampilan awal yang ikonik)
    return (
  ...
  return (
    <div className={`flex h-[100dvh] w-full font-sans overflow-hidden relative ${theme.bgBase} ${theme.textMain} ${printType === 'receipt' || printType === 'reprint' ? 'print-mode-receipt' : printType === 'report' ? 'print-mode-report' : ''}`}>
      ...
      <div id="printable-receipt" className="hidden bg-white z-[9999] p-2 text-black">
        {/* (Logika cetak struk thermal dipindahkan sebagian besar ke utilitas, HTML dasar tetap di sini agar tidak merusak format lama) */}
      ...
      {/* ========================================================= */}
      {/* MODALS GLOBAL (DIBIARKAN DI APP.JSX AGAR STATE AMAN) */}
      {/* ========================================================= */}
